...
function [theta, coll_frac] = CSMA_noncollocated_twocca_complex(N,K,L,p,T)
CCA1_fail = zeros(1,N);
CCA2_fail = zeros(1,N);
coll_ind = zeros(N,N);
coll =  zeros(1,N);
...
for t = 1:T
    ...
    for i = 1:N
        ...
        if (X(2,i) == -1) && (sense1_flag(i) == 0) % cca 1 success
            X(1,i) = -2;
        end
        ...
        if (X(2,i) == -2) && (sense2_flag(i) == 0) % cca 2 success
            X(1,i) = 1;
            ...
            if (mod(X(1,i),K) == 1)&&(t > coll_time(i)-1)  
                for j = [1:i-1 i+1:N]
                    if tau(j,i) >= 0
                        ...
                        coll_ind(j,i) = (X(1,j) == -2)*(sense2_flag(j) == 0) + (X(1,j) > 0)*(X(1,j) < K*L)*(mod(X(1,j),K) == 0)*(packet_cycle(j) > 0);
                    end
                end
                if sum(coll_ind(:,i)) > 0
                    coll(i) = coll(i) + 1;
            ...
            if (mod(X(1,i),K) == 1)&&(t > coll_time(i)-1)  
                for j = [1:i-1 i+1:N]
                    if tau(j,i) >= 0
                        ...
                        coll_ind(j,i) = (X(1,j) == -2)*(sense2_flag(j) == 0) + (X(1,j) > 0)*(X(1,j) < K*L)*(mod(X(1,j),K) == 0) ;
                    end
                end
                if sum(coll_ind(:,i)) > 0
                    coll(i) = coll(i) + 1;
...
coll_frac = sum(coll)/sum(packet_cycle);
end