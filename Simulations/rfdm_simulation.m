%% trying the IF Frequency calculation in twoDimFFTUA_RFDM code

% Define parameters
fs = 1000;          % Sampling frequency (Hz)
t = 0:1/fs:1;       % Time vector (0 to 1 second)
freq1 = 12.5;          % Frequency of the first ramp waveform (Hz)
freq2 = 12;          % Frequency of the second ramp waveform (Hz)
delay = 0.01;        % Time delay between the two waveforms (seconds)

% Generate the first ramp waveform
waveform1 = sawtooth(2*pi*freq1*t) + 1;

% Generate the second ramp waveform with time delay
waveform2 = sawtooth(2*pi*freq2*(t)) + 1;
...
frequency_difference = waveform1 - waveform2;
